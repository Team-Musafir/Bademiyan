...
export default function DestinationPage({ destinationData }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Full Width Image with Clip Path */}
      <motion.div 
      ...
      >
        ...
        >
          <img
            ...
            alt={`${destinationData.location} hero image`}
      ...
      >
        ...
        >
          ...
          <span className="text-lg sm:text-xl font-semibold">${destinationData.price}</span>
        </motion.div>
        <motion.div 
          className="flex justify-between items-center mb-4"
          variants={fadeInUp}
        >
          ...
          <span className="text-lg sm:text-xl font-medium text-black underline">{destinationData.quota} Available</span>
        </motion.div>
        <motion.div 
      ...
      {/* Enhanced Travel Itinerary Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-18 lg:mb-18 gap-4">
          ...
          >
            Your detailed day-by-day itinerary for the perfect {destinationData.location.split(',')[0].toLowerCase()} getaway
          </motion.p>
        </div>
        ...
        >
          {destinationData.itineraryDays.map((day, index) => (
            <motion.div
              key={index}
              variants={fadeInUp}
              className={`bg-white rounded-lg overflow-hidden transition-all duration-300 border-2 ${expandedDay === index
                  ? 'border-black'
            ...
            >
              ...
              >
                <div className="flex flex-col md:flex-row">
                  {/* Activities List - moved more to right */}
                  ...
                  {/* Image - kept original smaller size */}
      ...
      <motion.div 
        className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-[8rem]"
        initial="hidden"
        whileInView="visible"
        variants={fadeIn}
        viewport={{ once: true }}
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 h-auto md:h-96">
          ...
          {/* Right side images - takes 1 column, split into 2 rows */}
          <div className="grid grid-rows-2 gap-5 h-auto md:h-full">
            {/* Top right image */}
